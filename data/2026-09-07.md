...
TL;DR: A data-efficient framework for fine-grained timestamp-level VLA (Vision-Language-Action) failure detection that uses unlabeled action chunks for weak supervision and active learning for annotation selection, improving both timestamp-level and trajectory-level detection performance.


<details>
  <summary>Details</summary>
Motivation: VLA policies for robotic manipulation can fail unpredictably during long-horizon execution. Existing methods have limitations: (1) visual models detect failures only after erroneous actions occur (reactive), and (2) proactive detectors trained on VLA internal representations often use trajectory-level labels, causing normal pre-failure behavior in unsuccessful trajectories to be incorrectly labeled as failure. This supervision mismatch introduces label noise and limits detection accuracy and precise timestamp-level localization.

Method: 1. Leverages unlabeled VLA action chunks to construct action-derived weak supervision signals capturing abnormal patterns: inconsistent consecutive chunks, frozen/idle actions, and aggressive random motions. 2. Uses active learning to select only the most uncertain trajectories for timestamp-level annotation. 3. Fine-tunes the detector with these informative labels.

Result: Experiments across multiple VLA policies demonstrate that the method improves both timestamp-level and trajectory-level failure detection performance.

Conclusion: The proposed framework addresses the cost of dense annotation challenges while achieving superior failure detection capabilities at both granular timestamp and trajectory levels, making VLA policies more reliable for safe deployment in long-horizon robotic manipulation tasks.
...
TL;DR: Conductor is an edge-based solution that creates a unified world model from RSU perspective and predicts AV trajectories while respecting AoI constraints, using an occlusion-aware selector and runtime controller to adaptively fuse up to 31 CAVs.


<details>
  <summary>Details</summary>
Motivation: AVs have limited sensor line-of-sight due to traffic occlusions; existing per-CAV fusion approaches limit scalability and quality; edge-assisted unified world models can enhance planning but must respect tight AoI constraints.

Method: Conductor uses two key components: (1) Occlusion-aware selector that prioritizes AVs detecting RSU-covered objects; (2) Runtime controller that adaptively adjusts number of fused inputs and prediction amounts per cycle to stay within AoI budget.

Result: On CAV simulation infrastructure, Conductor meets AoI safety bounds with up to 31 CAVs, achieves fusion fidelity close to Oracle, and outperforms random selector under same AoI constraint.

Conclusion: Edge-based unified world modeling with AoI-aware adaptation enables scalable CAV collaboration for improved AV planning quality while respecting strict time budgets.
...
TL;DR: AquaBEV is a monocular underwater occupancy model that predicts local bird's eye view (BEV) occupancy from single underwater RGB images using 3D imaging sonar as geometric supervision during training.


<details>
  <summary>Details</summary>
Motivation: Autonomous underwater robots require understanding surrounding free and occupied space for safe navigation. Bird's eye view (BEV) occupancy provides this representation, but predicting it from a single underwater RGB image is difficult due to limited geometric cues from appearance alone. 3D imaging sonar offers complementary geometric measurements to address this challenge.

Method: AquaBEV maps visual features into a calibration-free polar representation and applies causal decoding along the range dimension before reconstructing predictions in Cartesian BEV coordinates. The researchers established a controlled underwater occupancy benchmark by adapting representative occupancy methods to the same RGB-to-sonar task under a unified protocol.

Result: AquaBEV achieves 31.4 Visible IoU and 38.6 Observed IoU, representing 4.0% and 4.3% relative improvements over the strongest transferred baseline.

Conclusion: The results demonstrate that monocular underwater occupancy prediction can be effectively supervised using 3D imaging sonar, significantly outperforming baseline methods and providing a practical solution for safe underwater navigation.
...
TL;DR: SocioGesture is a real-time adaptive social gesture perception system for HRI that uses a confidence-aware body-hand skeleton representation and lightweight dual-stream model to recognize social cues (invitations, refusals, unavailability) from noisy onboard perception under occlusion and latency constraints, with occlusion-aware training and online adaptation capabilities.


<details>
  <summary>Details</summary>
Motivation: Robots must recognize not only explicit commands but also social cues (invitations, refusals, unavailability) in real deployments. These cues must be inferred from noisy onboard perception under partial occlusion, changing viewpoints, and strict latency constraints, requiring robust and efficient social perception capabilities.

Method: SocioGesture employs: (1) compact confidence-aware body-hand skeleton representation; (2) lightweight dual-stream model fusing body motion with hand articulation for low-latency onboard recognition; (3) occlusion-aware skeleton corruption training (missing hands, occluded arms, temporally unstable keypoints); (4) uncertainty-based online adaptation with offline labeling for vocabulary expansion.

Result: On a social gesture dataset in mixed indoor-outdoor HRI scenarios, SocioGesture achieves: strong held-out-subject recognition; substantially improved robustness under structured joint occlusion; real-time performance on robot-mounted edge devices; and the ability to expand gesture vocabulary while preserving performance in original classes through online adaptation.

Conclusion: Demonstrates a practical path toward robust, efficient, and adaptive social perception for interactive robots, enabling real-time recognition of social cues in challenging real-world conditions with minimal computational overhead.
...
TL;DR: CFAM is a continual learning system that uses a complementary learning architecture with a frozen slow-learning component (Sensor, Reasoning, Action cortices) and a fast-learning Capsule Field to enable efficient on-device learning in mission-critical operations with scarce training data.


<details>
  <summary>Details</summary>
Motivation: Unattended interactive autonomy for mission-critical operations is missing; these domains have scarce training data and only onboard compute, yet deployed systems must face novelty without erasing prior competence.

Method: Continual Field-Adaptive Models (CFAMs) use a complementary learning architecture with a frozen slow-learning component and a fast-learning Capsule Field that stores field learning one-shot and gradient-free as Competence Capsules. Skill installation is few-shot in lab and continual in field.

Result: CFAM reaches operating point of a standard policy using 40% of data or 2.5x fewer trajectories. At test time, autonomous capture of verified near-OOD cases improves action success by 13.9 percentage points. In sequential simulation, backward transfer is -0.5 percentage points versus -11.4 for LoRA.

Conclusion: CFAM provides a bounded form of post-deployment physical intelligence: few-shot skill learning, autonomous field growth from verified near-OOD experience, and retention of prior competence.
...
TL;DR: NavArena transforms fixed 3D Gaussian Splatting reconstructions into benchmarks for goal-oriented visual navigation by integrating frozen 3DGS rendering, occupancy costmaps from Gaussian density, and open-vocabulary semantic goal candidates. It generates 22.2 million expert trajectories across 2,000+ scenes and provides a unified closed-loop evaluation protocol for scalable navigation assessment.


<details>
  <summary>Details</summary>
Motivation: Fixed 3D Gaussian Splatting reconstructions provide realistic novel views but lack traversability constraints, valid goals, and closed-loop protocols required for navigation evaluation.

Method: NavArena integrates three key components: (1) a frozen 3DGS model for egocentric RGB-D rendering, (2) an occupancy costmap derived from Gaussian density and height statistics for reachability and collision queries, and (3) semantic goal candidates lifted from multi-view open-vocabulary masks. This framework supports automatic generation and unified closed-loop evaluation of goal-oriented navigation episodes.

Result: Across more than 2,000 scenes, NavArena generates 22.2 million expert trajectories. Spatial and semantic evaluations assess the derived navigation representations, while policy rollouts demonstrate the diagnostic value of the unified evaluation protocol.

Conclusion: NavArena enables scalable and reproducible navigation evaluation on large-scale 3DGS reconstructions. All benchmark-generation tools, evaluation protocols, and derived assets will be released publicly.
...
TL;DR: A diffusion-based visuomotor policy learns dressing assistance from static demonstrations and generalizes to dynamic human motion by approximating arm trajectories through object-centric PDE diffusion on partially observed point clouds.


<details>
  <summary>Details</summary>
Motivation: Robotic dressing assistance for older adults is promising but challenging due to complex garment-human contact and occlusions that make it difficult to generate actions aligned with arm movements during dynamic dressing scenarios.

Method: A diffusion policy tailored to garment-human interaction geometry is trained on static expert demonstrations and partially observed point clouds with varied arm postures. The method uses an object-centric representation based on PDE diffusion to capture the axial distribution of the arm, samples motion-relevant regions, and registers them across consecutive observations to approximate arm motion and reactively adapt trajectories.

Result: Evaluated in simulation and a real-world study with 9 participants, 3 garment types, and 6 arm-motion patterns. The method outperforms baselines in dressing progress, freedom of movement, and user comfort metrics.

Conclusion: The proposed visuomotor policy successfully generalizes from static demonstrations to dynamic human motion scenarios for robotic dressing assistance, providing a viable solution for supporting older adults with physical impairments in daily living.
...
TL;DR: HaptiNet is a networked haptic robotic system enabling physical co-presence between geographically distributed users through force-mediated interaction for remote rehabilitation, validated across 284 healthy participants and 111 patients with neurological impairments.


<details>
  <summary>Details</summary>
Motivation: Cooperative rehabilitation enhances engagement and task performance but requires physical co-presence, while current telerehabilitation remains audiovisual, leaving users haptically and physically isolated. There is a need to enable force-mediated interaction across distances to maintain the benefits of physical co-presence in remote rehabilitation settings.

Method: HaptiNet features low-inertia, long-stroke robotic terminals with high force-feedback capacity for upper-limb haptic rendering. It uses an imitation-learning-based delay compensator to create a distributed haptic network. Validation included 284 healthy participants and 111 patients with neurological impairments across laboratory tests, cross-city deployments, and clinical applications.

Result: HaptiNet preserved task-level force rendering consistency across single-user and multi-user scenarios. Compared to solo and visual cooperative training, haptic cooperation improved task performance by 24% and 22%, respectively, while boosting engagement and interpersonal motor synchrony. Across three intercity links (4,000 km), HaptiNet maintained stable haptic interaction, producing 3.87-fold greater baseline-to-training score improvement and 106% higher patient-applied effort versus the solo condition.

Conclusion: HaptiNet successfully enables physical co-presence for geographically distributed rehabilitation, significantly outperforming solo and visual cooperative training in task performance, engagement, and motor synchrony while maintaining stable force rendering across long-distance connections.
...
TL;DR: CoLMIN is an LLM-based multi-decision path negotiation framework for cooperative autonomous driving that addresses premature convergence to suboptimal solutions by combining multi-intent negotiation with shallow and deep reflection mechanisms, achieving superior consensus performance in CARLA simulations.


<details>
  <summary>Details</summary>
Motivation: Multi-vehicle cooperative autonomous driving improves safety through information sharing, and LLM-based approaches enable effective inter-vehicle negotiation. However, driving decisions are inherently multi-solution, causing existing methods to converge prematurely to suboptimal solutions, hindering consensus formation and limiting practical deployment.

Method: CoLMIN consists of three components: (1) LMin - LLM-based Multi-Intent Negotiation module using a Negotiator-Evaluator paradigm to generate multiple candidate driving intentions; (2) ESRM - Evaluation-based Shallow Reflection Module that analyzes negotiation outcomes and provides feedback to accelerate consensus; (3) LDRM - LLM-based Deep Reflection Module that performs long-term reflection over negotiation histories to mitigate cognitive fixation and prevent suboptimal convergence.

Result: Experimental results in the CARLA simulation environment demonstrate that CoLMIN significantly outperforms existing methods in challenging interactive driving scenarios, achieving stable and high-quality decision consensus.

Conclusion: CoLMIN effectively addresses the premature convergence problem in cooperative autonomous driving by leveraging multi-decision path negotiation and reflective reasoning, making it a promising approach for practical deployment of cooperative autonomous driving systems.
...
TL;DR: This paper designs a potential function that densely fills the configuration space of conservative mechanical systems with strict nonlinear normal modes (SNMs), enabling efficient periodic trajectory execution with minimal actuator torque requirements.


<details>
  <summary>Details</summary>
Motivation: Strict nonlinear normal modes provide regular oscillations but typically exist as isolated curves in configuration space. The authors aim to overcome this limitation by creating a potential that densely fills configuration space with SNMs, where each configuration belongs to a mode and all modes pass through a common equilibrium point.

Method: The authors design a potential that can be realized by nonlinear elastic elements, which enables dense SNM coverage of configuration space. They implement three controllers: (1) a stabilizer to reach a desired target mode, (2) a swing-up controller, and (3) a dissipation compensator. The approach is demonstrated on a 2-degree-of-freedom (2-DOF) manipulator.

Result: Experimental implementation on a 2-DOF manipulator demonstrates successful execution of periodic trajectories. Most required torques come from elastic elements rather than actuators, showing high efficiency.

Conclusion: The proposed method effectively fills configuration space with strict normal modes, enabling efficient periodic motion execution with minimal actuator effort. The approach successfully stabilizes to target modes, performs swing-up, and compensates for dissipated energy in practical robotic systems.
...
TL;DR: Latent Semantic Scaffolding (LSS) aligns VLA action-token representations with physical-reasoning rationales during training through a projection head, with Dense LSS (per-phase alignment) outperforming Pooled LSS (episode-level alignment) in both in-distribution success and transfer to unseen tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLA models trained by imitation capture what to do but not why. Adding causal reasoning improves manipulation but is expensive at inference (generating reasoning tokens or rolling out predicted future states). The authors ask whether this benefit can be captured during training and discarded before deployment.

Method: Latent Semantic Scaffolding (LSS) introduces an auxiliary loss during human-demonstration pretraining that aligns a VLA's action-token representations to text embeddings of physical-reasoning rationales through a small projection head, which is dropped at inference. Two alignment strategies are compared: Dense LSS (aligning each action token to its own manipulation phase rationale) and Pooled LSS (aligning to a single pooled episode-level embedding).

Result: Dense LSS achieves both the best in-distribution success rate and the best transfer to tasks unseen during alignment. Pooled LSS over-specializes to the training task. A representational probe demonstrates that Dense LSS induces roughly twice the per-phase separability in the backbone network.

Conclusion: Phase-local alignment is the operative mechanism. By aligning action tokens to their specific manipulation phase rationales rather than aggregated episode-level embeddings, LSS captures causal reasoning benefits during training at zero inference cost, improving both performance on seen tasks and generalization to new tasks.
...
TL;DR: ToPos is an automated framework for optimal spatial sampling on topographic manifolds using geodesic distances and constrained geodesic Voronoi decomposition, achieving 74% improvement in surface area-balanced distribution compared to conventional 2D Euclidean methods.


<details>
  <summary>Details</summary>
Motivation: Conventional 2D Euclidean methods fail in high-relief environments by neglecting topographic variations and physical obstructions, leading to planimetric distortion, spatial clustering, and placement of targets in inaccessible or shadowed regions, which compromises data integrity and operational safety for applications like autonomous mapping, environmental sampling, last-mile logistics, and infrastructure deployment.

Method: The terrain is treated as a discrete 2D manifold embedded in 3D Euclidean space, replacing standard flat-map distances with non-Euclidean geodesic distances. The point distribution is formulated as a constrained geodesic Voronoi decomposition optimization problem solved via Riemannian Nesterov Accelerated Gradient (NAG), with target locations restricted to a feasible "safe zone" accounting for non-traversable slopes, vegetation, and environmental occlusions.

Result: Evaluated on non-convex sinusoidal manifolds, ToPos mitigates planimetric distortion by utilizing geodesic metrics, achieving approximately 74% improvement in optimal surface area-balanced distribution, as measured by the coefficient of variation (CV) of Voronoi cell areas.

Conclusion: The framework is architected as a Geographic Information System (GIS)-ready micro-service to support autonomous mapping, environmental sampling, last-mile logistics, and infrastructure deployment applications.
...
TL;DR: A decentralized navigation system for composite heterogeneous robots combining LLM policy agents, UCB bandits, and Double DQN controllers operating at different update levels, with evaluation showing the complete configuration achieves the lowest median completion time.


<details>
  <summary>Details</summary>
Motivation: Addresses navigation challenges in decentralized systems where policy reasoning and local control operate at different update levels, requiring a solution without central LLM generation of team actions.

Method: NetLogo-Python implementation with three robots sharing motion dynamics but using different LLM backends. Each robot independently combines an LLM policy agent (round-level), UCB bandit (refinement-mode selection), and policy-conditioned Double DQN (tick-level action selection). Cross-LLM communication occurs through shared round summaries containing policies, outcomes, and learning feedback.

Result: The complete configuration reached the goal in all 90 correlated robot-round records with the lowest median completion time (42 ticks) and P90 (73.2 ticks), which was 25.0-39.1% lower than other configurations.

Conclusion: Provides descriptive, configuration-level evidence that combining LLM policy agents, UCB bandits, and Double DQN controllers in a decentralized system yields superior navigation performance compared to isolated components.
...
TL;DR: A unified framework analyzing robotic hand kinematics and actuation separately is applied to two robotic hands (Shadow Dexterous Hand and Anatomically Correct, Biomechatronic Hand) to examine how joint axis geometry, actuator-to-DOF ratio, coupling architecture, and authority distribution affect control difficulty. The framework reveals that anatomical fidelity provides no uniform advantage: oblique axes improve thumb conditioning but worsen long finger conditioning, while branching tendon networks improve long finger control mapping but significantly degrade thumb control.


<details>
  <summary>Details</summary>
Motivation: Robotic hands vary widely in anatomical fidelity and mechanical complexity, and these structural choices influence the coordination of joint motions and the difficulty of controlling the system. There is a need for a unified framework to understand how different hand morphologies affect controllability.

Method: A unified framework is presented that analyzes kinematic and actuation stages separately through conditioning of the task Jacobian, actuation matrix, and their product. This framework is applied to two hands with opposing design philosophies, examining four morphological aspects: joint axis geometry, actuator-to-DOF ratio, coupling architecture, and authority distribution. All parameters are derived from the hands' canonical digital representations. Predictions are evaluated against reinforcement learning experiments using PPO, DDPG+HER, and TQC+HER algorithms across three different tasks.

Result: Anatomical fidelity carries no uniform advantage: oblique axes improve thumb conditioning but leave long fingers worse conditioned than the orthogonal-axis design, while the branching tendon network improves effective control mapping at every long finger but worsens it significantly at the thumb, where actuator authority is concentrated on thumb opposition.

Conclusion: The framework successfully predicts how different hand morphologies affect control difficulty, demonstrating that anatomical fidelity does not uniformly improve controllability and that structural choices have complex, finger-specific effects that must be carefully considered in robotic hand design.
...
TL;DR: A real-robot benchmark called "One Word, Different Action" that evaluates robot models' ability to handle instruction changes through task-preserving and task-changing instruction pairs, testing Decision Invariance and Decision Sensitivity under multi-constraint reasoning and real-RGB grounding.


<details>
  <summary>Details</summary>
Motivation: A reliable embodied system must preserve its action when the task is unchanged (Decision Invariance) and update it correctly when the task changes (Decision Sensitivity). The challenge is creating evaluation frameworks that can jointly assess both capabilities.

Method: The authors introduce a real-robot benchmark built on physical decision states and executable actions, using task-preserving and task-changing instruction pairs. The evaluation includes single-constraint and multi-constraint reasoning, as well as real-RGB grounding scenarios.

Result: Modern models show near-saturation performance on single-constraint instruction changes, but several models degrade noticeably when multiple task constraints must be integrated into a single executable decision.

Conclusion: The remaining challenge for robot instruction following is no longer recognizing isolated instruction changes, but reliably composing multiple task requirements into correct robot action decisions.
...
TL;DR: TacPAC introduces a real-time action correction system that caches predicted contacts and plan representations to enable efficient tactile feedback integration during execution, improving task success rates from 22% to 64% across five real-robot tasks.


<details>
  <summary>Details</summary>
Motivation: Vision-centric world-action models miss local contact cues essential for contact-rich manipulation, and naively adding tactile predictions only recovers a fraction of potential gains due to a timing mismatch between prediction (before execution) and tactile feedback (during execution).

Method: TacPAC caches predicted contacts and plan representations alongside planned action chunks. A tactile expert reads each newly observed tactile image against this cache to correct actions not yet executed, providing feedback interpretation against plan expectations rather than in isolation. This approach requires a single cache pass per correction (20.7× cheaper than regenerating action chunks).

Result: TacPAC achieves state-of-the-art performance on five real-robot tasks spanning precision insertion, fragile-object handling, object reorientation, and long-horizon manipulation, raising average task success from 22% (vision-only baseline) to 64%.

Conclusion: Integrating tactile feedback through cached plan-conditioned predictions enables efficient real-time action correction that significantly outperforms vision-only approaches and naive tactile prediction, demonstrating the importance of interpreting feedback in context of plan expectations.
...
### [33] [At Equal Inference Cost, Multi-Agent Structure Does Not Beat a Single Frozen Agent](https://arxiv.org/abs/2609.04217)
*David Dylan,Aoife Brennan,Cian Murphy,Niamh O'Sullivan,Conor Kelly,Saoirse Walsh*

Main category: cs.MA

TL;DR: Multi-agent LLM teams (Planner-Executor-Critic) often require more compute than single agents. This paper introduces MA-Evolve to optimize multi-agent teams under equal inference budgets and finds that multi-agent structure adds cost without clear benefit.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM pipelines (Planner-Executor-Critic teams) report gains over single agents, but at higher inference cost due to multiple model calls per environment step. Existing methods compare teams against single agents at equal environment rollouts, giving teams extra compute. The authors aim to fix the total number of language-model calls and test whether evolving a multi-agent team still beats evolving a single agent under the same budget.

Method: MA-Evolve represents a Planner-Executor-Critic team as three evolvable role prompts and optimizes them via per-role coordinate ascent over a shared frozen 7B backbone.

Result: On ALFWorld: evolving a single executor significantly improves over unevolved agent (0.769 vs 0.754, p=0.80), but the full team is not statistically better than the single agent despite using 1.8× more evaluation calls. Leave-one-in analysis shows value comes entirely from the executor; planner and critic evolve to empty/low-impact prompts and rarely change executor actions. On WebShop: evolution is null while the team trends worse.

Conclusion: Under equal inference cost, multi-agent structure adds cost without clear benefit. The planner and critic roles provide minimal value compared to a single optimized executor agent.
...
### [34] [Multi-dimensional Bias in Modeling Multi-dimensional Preferences: Evaluating the Ability of Synthetic Agents to Replace Human Participants in Conjoint Experiments](https://arxiv.org/abs/2609.04243)
*Ho Ting Hung,Nachiket Midha,Victor Y. Wu,Yiwen Zhang*

Main category: cs.MA

TL;DR: This paper investigates whether LLM-based synthetic agents can effectively replicate human preference patterns in conjoint experiments, finding uneven alignment across different dimensions of evaluation.


<details>
  <summary>Details</summary>
Motivation: Growing interest in using LLMs for survey experiments exists, but their efficacy in conjoint design—a popular method in political science—remains underexplored.

Method: The study replicates published conjoint studies and compares synthetic agents' results with original human data across three dimensions: representational correspondence, inferential correspondence, and procedural stability, evaluating choice distributions as well as statistical and substantive similarity of estimates.

Result: Results are uneven across dimensions and studies replicated, indicating that the validity of synthetic participants is claim-dependent and hierarchical.

Conclusion: Reproducing figures or obtaining strong sign agreement is insufficient to support replacing human respondents; the discipline must first map this innovation's boundaries across various levels before considering synthetic agents a robust substitute for human samples.
...
### [35] [EVOHARNESSBENCH: Can Your Agents Keep Pace with an Evolving Harness?](https://arxiv.org/abs/2609.04280)
...
### [36] [Abstraction Agent](https://arxiv.org/abs/2609.04303)
*Boning Li,Longbo Huang*

Main category: cs.MA

TL;DR: The Abstraction Agent is a zero-shot pipeline using large language models to discover continuous strategic features for game abstraction, eliminating the need for domain-specific evaluators and enabling scalable solution of large imperfect-information games.


<details>
  <summary>Details</summary>
Motivation: Information abstraction is essential for scaling game-solving algorithms to large imperfect-information games, but traditional methods require domain-specific evaluators (hand-strength calculators, equity estimators) that demand expert knowledge and engineering effort, making them unavailable for most less-studied games.

Method: A four-phase pipeline using large language models: (1) feature discovery with calibration anchors, (2) batched private-state scoring, (3) correlation-based feature selection, and (4) k-means clustering. The LLM discovers continuous strategic features from natural-language game descriptions, scores private states on these features, and clusters them into abstraction buckets without game-specific evaluators, training data, or game-tree traversal.

Result: On heads-up no-limit Texas hold'em turn endgames, the abstractions reduce lifted-strategy exploitability by up to 62% relative to expected-hand-strength baseline. On ROVER Trials (a novel game), the approach beats scalar rank baseline at every granularity. The pipeline transfers to four other games (four-card Pot-Limit Omaha, HUNL preflop/flop, Riichi Mahjong) with unchanged prompts, where discovered features track recognized strategic concepts.

Conclusion: The Abstraction Agent demonstrates structured knowledge elicitation—converting implicit strategic knowledge in LLM parameters into explicit numerical features for downstream algorithmic computation—achieving strong performance across diverse games without domain-specific engineering.
...
### [37] [Online Change-point Detection for Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2609.05298)
*Fatemeh Saberi Khomami,Julita Vassileva*

Main category: cs.MA

TL;DR: This paper introduces PPR, a lightweight online change-point detection algorithm for cooperative MARL that smooths reward streams and uses statistical drift detection to identify significant shifts in task objectives or environments during training.


<details>
  <summary>Details</summary>
Motivation: Cooperative MARL systems rely on past experience for coordinated behavior, but when the environment or task objective changes during training, this experience becomes unreliable. Agents need to first recognize that a situation has changed before deciding how to adapt.

Method: Patterns of Past Rewards (PPR): A lightweight, algorithm-agnostic detector that smooths agents' return streams, highlights recent changes, and applies a statistical drift detector to flag significant shifts in reward patterns.

Result: Evaluated in a Speaker-Listener environment (based on Multi-Agent Particle Environment) under two non-stationarity scenarios. Found a trade-off between detection speed and alarm stability: smoothed-return baseline detects earlier but produces many repeated alarms, while direct detection on raw returns often misses shifts. PPR offers a balanced approach limiting redundant detections while still identifying controlled shifts.

Conclusion: PPR serves as a lightweight, reward-based monitoring tool that enables cooperative MARL systems to reliably identify major changes during training, improving adaptability when task dynamics change.
...
### [38] [Trust-Aware Adaptive Disclosure for Inference Privacy Preservation in Multi-Agent Networks](https://arxiv.org/abs/2609.05340)
*Puspanjali Ghoshal,Tobias J. Oechtering*

Main category: cs.MA

TL;DR: A multi-agent system with hidden goals uses a Trust-Aware Privacy Control framework that adapts message disclosure via trust-dependent stochastic policies, trading off consensus performance against privacy preservation under goal inference attacks.


<details>
  <summary>Details</summary>
Motivation: Agent-based systems are increasingly deployed in information-critical domains like healthcare and smart grids, creating a need for privacy-preserving consensus in networked multi-agent systems under goal inference attacks.

Method: Proposes a Trust-Aware Privacy Control framework that adapts message disclosure based on dynamic trust relationships between agents using a trust-dependent stochastic policy to control information release.

Result: Experiments demonstrate the proposed method reduces adversarial goal inference accuracy compared to representative baselines while maintaining competitive consensus utility.

Conclusion: Trust-aware mechanisms are effective for privacy preservation in multi-agent systems, achieving a beneficial tradeoff between consensus performance and privacy.
...
### [39] [STyMo: Fast and Controllable Few-Shot Motion Style Transfer](https://arxiv.org/abs/2609.04500)
...
### [40] [LayoutShop: Content-Constrained Exploratory Design of Creative Article Layout](https://arxiv.org/abs/2609.05098)
*Jialuo Li,Pengfei Xu*

Main category: cs.GR

TL;DR: LayoutShop is a computational framework for designing creative article layouts that helps users frame a given article through template-based conceptualization, geometry optimization, and neural network assessment.


<details>
  <summary>Details</summary>
Motivation: The framework is inspired by the actual article layout design process, aiming to enable users to create or select layout templates to construct a layout design space and determine geometry via optimization to frame articles.

Method: The method enables users to create or select layout templates for conceptualization, constructs a layout design space to extract eligible layout structures, determines geometry via optimization, employs two neural networks for layout assessment, and returns high-quality outputs for user selection.

Result: A user study confirmed that LayoutShop effectively helps users create high-quality article layouts, demonstrating the framework's usability and effectiveness.

Conclusion: LayoutShop successfully provides an effective solution for designing creative article layouts, as validated by user study results showing improved layout quality and usability.
...
### [41] [GradRig: Differentiable Weights for Skinned Gaussian Splat Deformation](https://arxiv.org/abs/2609.05127)
*Nina Vesseron,Élie Michel*

Main category: cs.GR

TL;DR: This paper addresses skinning deformation for 3D Gaussian Splats by using skinning weight gradients to create a mesh-free deformation pipeline that prevents holes when stretching, while maintaining real-time rendering capabilities.


<details>
  <summary>Details</summary>
Motivation: Skinned deformation is a common technique for 3D shapes that works by deforming a kinematic structure (rig). For 3D meshes, this is straightforward - vertices are displaced to deform connecting polygons. However, for 3D Gaussian Splats, which lack connectivity information, rigidly transforming points is insufficient to prevent holes when stretching shapes. The authors need a solution that works with Gaussian Splats without sacrificing real-time rendering performance.

Method: The authors propose using the spatial gradient of skinning weights to create a mesh-free deformation pipeline for Gaussian Splats. The approach includes: (1) evaluating gradients when the user creates the rig structure, and (2) proposing an optional adaptive resampling scheme to split splats that still produce artifacts. This allows for more accurate stretching of splats compared to rigid transformations.

Result: The proposed skinning weight gradient approach successfully provides accurate deformation of Gaussian Splats without requiring mesh connectivity information. The method prevents the creation of holes when stretching shapes, and maintains real-time rendering capabilities as demonstrated in their WebGL viewer.

Conclusion: The paper demonstrates that skinning weight gradients provide an effective solution for deforming 3D Gaussian Splats in a mesh-free manner. This approach enables accurate shape stretching while preserving the performance benefits of Gaussian Splatting's real-time rendering capabilities, making it practical for interactive applications.
